...
@Mod(modid = EntityCulling.MOD_ID, acceptableRemoteVersions = "*", dependencies = "required-after:renderlib@[1.4.5,)", guiFactory = "meldexun.entityculling.config.EntityCullingConfigGuiFactory")
public class EntityCulling {
	...
	public static boolean isHatsInstalled = false;
	...
	public void onFMLConstructionEvent(FMLConstructionEvent event) {
		System.out.println("[EntityCulling] Does use opengl based culling : "+useOpenGlBasedCulling());
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		configFile = new File(event.getModConfigurationDirectory(), MOD_ID + ".cfg");

		try {
				EntityCullingConfig.loadConfig(configFile);	//load config
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		Runtime.getRuntime().addShutdownHook(new Thread(() -> {
			try {
				EntityCullingConfig.saveConfig();	//save config on shutdown
			} catch (Exception e) {
				e.printStackTrace();
			}
		}));
