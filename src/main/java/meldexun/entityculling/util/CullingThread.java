...
public class CullingThread extends Thread {
	...
	private final RaytracingEngine engine = new RaytracingEngine(EntityCullingConfig.cacheSize, (x, y, z) -> {
		return this.cachedBlockAccess.getBlockState(x, y, z).getBlock().isOpaqueCube();
		...
		while (true) {
			...
			try {
				...
				if (world != null && player != null && renderViewEntity != null) {
					...
					float partialTicks = mc.timer.renderPartialTicks;//mc.getRenderPartialTicks();
					...
					Vec3 cameraPosition = ActiveRenderInfo.getPosition();//.getCameraPosition();
					this.camX = this.x + cameraPosition.xCoord;
					this.camY = this.y + cameraPosition.yCoord;
					this.camZ = this.z + cameraPosition.zCoord;
	...
	private boolean checkEntityVisibility(Entity entity) {
		...
		// 	return false;
		// }
		if (EntityCullingConfig.entity.alwaysRenderBosses && (entity instanceof EntityDragon || entity instanceof EntityWither)) {	//!isNonBoss
			return true;
		}
		...
		if (EntityCullingConfig.entity.ignoreEndCrystalsWithBeam && entity instanceof EntityEnderCrystal){// && ((EntityEnderCrystal) entity).getBeamTarget() != null) {	no beamtarget in 1.8.9 :()
			return true;
		}
	...
	private boolean checkEntityShadowVisibility(Entity entity) {
		...
		if (!EntityCulling.useOpenGlBasedCulling()) {
			...
			if (EntityCullingConfig.entity.alwaysRenderBosses && (entity instanceof EntityDragon || entity instanceof EntityWither)) {	//!isNoneBoss
				return true;
			}
			if (EntityCullingConfig.entity.alwaysRenderEntitiesWithName && entity.getAlwaysRenderNameTagForRender()) {
				return true;
			...
			if (EntityCullingConfig.entity.ignoreEndCrystalsWithBeam && entity instanceof EntityEnderCrystal){// && ((EntityEnderCrystal) entity).getBeamTarget() != null) {    same reason as above it dont exist in 1.8.9
				return true;
			}
			if (EntityCullingConfig.entity.skipHiddenEntityRenderingBlacklistImpl.get(entity)) {
				return true;
	...
	private boolean checkBoxCached(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
		int startX = MathHelper.floor_double(minX);
		int startY = MathHelper.floor_double(minY);
		int startZ = MathHelper.floor_double(minZ);
		int endX = MathHelper.ceiling_double_int(maxX);
		int endY = MathHelper.ceiling_double_int(maxY);
		int endZ = MathHelper.ceiling_double_int(maxZ);
	...
	private boolean checkBoxUncached(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
		...
		int stepsX = MathHelper.ceiling_double_int(deltaX);
		int stepsY = MathHelper.ceiling_double_int(deltaY);
		int stepsZ = MathHelper.ceiling_double_int(deltaZ);
